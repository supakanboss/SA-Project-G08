...
function CreateBorrow_Sport_Equipment() {
  ...
  const [Time_Giveback, setTime_Giveback] = React.useState<Date | null>(
    new Date()
  );
  // combo box
  const [Sport_Equipment_Type, setSport_Equipment_Type] = useState<
    SportEuqipmentTypeInterface[]
  >([]);
  const [Sport_Equipment, setSport_Equipment] = useState<
    SportEquipmentInterface[]
  >([]);
  const [CreateBorrow_Sport_Equipment, setCreateBorrow_Sport_Equipment] =
    useState<Partial<CreateBorrow_Sport_EquipmentInterface>>({});
  const [member, setMember] = useState<MemberInterface>();
  const [CreateBorrow_Sport_Equipments, setCreateBorrow_Sport_Equipments] =
    React.useState<CreateBorrow_Sport_EquipmentInterface[]>([]);
  ...
  function submit() {
    let data = {
      ...
      Sport_Equipment_TypeID: convertType(
        CreateBorrow_Sport_Equipment.Sport_Equipment_TypeID
      ),
      Sport_EquipmentID: convertType(
        CreateBorrow_Sport_Equipment.Sport_EquipmentID
      ),
